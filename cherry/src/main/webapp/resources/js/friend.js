         ...
         $.ajax({
            ...
            success : function(data){
            	...
            	for(var i = 0 ; i < data.length ; i++){
            		if(data[i].yok == '1'){
                      content += '<tbody>';
                      content += '<tr id="'+data[i].yaman_id+'" style="background-color: skyblue; text-align:center;"  class="detail_data" >';  
                      
                      content += '<td>';
                      if(data[i].yface == '없음'){
                         content += '사진이 없습니다.';
                      }else{
                         content += '<img src="resources/images/'+data[i].yface+'" style="width: 30px; height: 30px;" class="img-circle person">'; 
                      }
                      ...
                      content += '<tbody id="fd_tbody">';
                      content += '<tr id="'+data[i].yaman_id+'" style="background-color: pink; text-align:center;" class="detail_data">';  
       ...
       $.ajax({
          ...
          success : function(brkMessage){
             var content = '<table class="table" id="fdAddTable">'
                ...
                +'<th>확인</th>'+'</thead>';
             if(brkMessage.length == 0){
          	   content += '<tbody>'; 
          	   content += '<tr>'+'<td></td>'+'<td>'+'<span id="no_brk_message">'+'절교 메세지가 없습니다.'+'</span>'+'</td>'+'<td></td>'+'</tr>';
          	   content += '</tbody>';
             } else{
...
//         $("#myUpdateModal input").css("border-style" , "2px dotted orange");
           ...
           $.ajax({
                ...
                success : function(reqMessage){
                   ...
                   if(reqMessage.length == 0){
                	   content += '<tbody>';
                	   content += '<tr>'+'<td>'+'<span id="no_req_message">'+'친구요청이 없습니다.'+'</span>'+'</td>'+'</tr>';
                	   content += '</tbody>';
                   }else{
