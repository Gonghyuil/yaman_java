	    	...
	    	$.ajax({
	    		...
	    		success : function(data){
	    			for( i=0; i< data.length; i++){
	    				allimg += '<div float:left; style="float: left;">'+'<img src="/cherry/resources/images/' +data[i].yphotofile +'"class='+'"img-thumbnail"'
						...
						var replyFrom = 
							...
							'<div margin="50">'+
						    	...
						    	'<input class="form-control input-sm"' + 'id="'+data[i].yphoto_id+'"'+ 'type="text" onKeyPress="if (event.keyCode==13) {reply(' +data[i].yphoto_id +')}">'+
						    	'</label>'+
						    	'<button type="button"'+'class="btn btn-link"' +'onClick="reply('+data[i].yphoto_id +')">댓글입력</button>'+
							...
							'<div class="container">'+
		    					'<table class="table table-striped">'+
		    						'<thead>'+
		    							'<tr>'+
		    								'<th width="50px;">이름</th><th width="50px;">내용</th><th>삭제</th>'+
		    							'</tr>'+
		    						'</thead>'+
    			...
    			for(i=0; i< data.length; i++){
    				for(j=0; j< photoid.length; j++){
    					if( photoid[j] ==data[i].yphoto_id ){
    						var div =
    			    						'<tr id="deltr'+data[i].yreply_id+'">'+
    			    							'<td>' +data[i].yname +'</td>'+
    			    							'<td>' +data[i].ytitle+'</td>'+
    			    							'<td><span class="glyphicon glyphicon-remove" onclick="delrep('+data[i].yreply_id +')"></span></td>'+
    			    						'</tr>';
	...
	function aaa(promise_id){
		return function(){
			...
			$.ajax({
				...
				success: function(data){
					for( i=0; i< data.length; i++){
						images = '<div float:left; style="float: left;" id="d'  +data[i].yphoto_id +'"><img src="/cherry/resources/images/' +data[i].yphotofile 
						+'"class='+'"img-thumbnail"'
		...
		$.ajax({
			...
			success: function(data){
				...
				for( i=0; i< data.length; i++){
					...
					+'<img src="/cherry/resources/images/' +data[i].yphotofile +'"'
